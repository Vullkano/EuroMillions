...
Impar = 1  # Exemplo: A Soma dos números será impar? (Sim -> 1, Não -> 0)
...
# Arredondar os valores previstos para inteiros e ajustar para o intervalo 1-50
previsao_inteira = np.round(previsao).astype(int)
previsao_inteira = np.clip(previsao_inteira, 1, 50)
previsao_inteira = np.sort(previsao_inteira)
